...
<main class="container px-4">
    <div class="col-lg-8 mx-auto">
        <div class="bg-light rounded-4 shadow p-4 text-center">
            ...
            @if ($kuis_id == 4)
                {{-- Halaman evaluasi --}}
                <h2 class="fs-4 mb-3 text-primary">Selamat! Kamu telah menyelesaikan Evaluasi</h2>
                <hr>
                <p><strong>Nama:</strong> {{ $nama }}</p>
                <p><strong>Hari:</strong> {{ $hari }}</p>
                <p><strong>Tanggal:</strong> {{ $tanggal }}</p>
                <p><strong>Waktu Pengerjaan:</strong> {{ $waktu }}</p>
                <p><strong>Skor Evaluasi:</strong> {{ $skor }}</p>

                <p><strong>Feedback Evaluasi:</strong>
                    @if ($status === 'memenuhi')
                        <span class="text-success">Selamat! Kamu telah lulus evaluasi.</span>
                    @else
                        <span class="text-danger">Maaf, kamu belum lulus evaluasi.</span>
                    @endif
                </p>
